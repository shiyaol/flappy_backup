...
def train_agent(iter_num=iter_num, im_size=image_size, b_size=batch_size, lr=learning_rate, gamma=gamma,
                m_size=memo_size, m_path=model_path):
    ...
    while iter < iter_num:
        ...
        y_batch = torch.cat(tuple(r if t else r + gamma * torch.max(next_p) for r, t, next_p in
                                  zip(rewards, terminal_batch, next_prediction_batch)))
